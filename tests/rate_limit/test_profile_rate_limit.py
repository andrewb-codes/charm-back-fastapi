...
async def test_update_profile_applies_profile_write_rate_limit(client: AsyncClient) -> None:
    token = await register_and_login(client)
...
async def test_update_profile_returns_429_when_write_rate_limit_exceeded(
    client: AsyncClient,
) -> None:
    token = await register_and_login(client)
...
async def test_change_email_applies_profile_write_rate_limit(client: AsyncClient) -> None:
    token = await register_and_login(client)
...
async def test_change_password_applies_profile_write_rate_limit(client: AsyncClient) -> None:
    token = await register_and_login(client)
