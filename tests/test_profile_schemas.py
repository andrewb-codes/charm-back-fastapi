...
from charm.models.profile import Gender, Role, Status
...
def test_profile_update_accepts_birthdate_in_past() -> None:
    ...
    assert request.birthdate == birthdate
...
def test_profile_update_rejects_birthdate_that_is_not_in_past(birthdate: date) -> None:
    with pytest.raises(ValidationError, match="birthdate must be in the past"):
        ProfileUpdateRequest.model_validate({"birthdate": birthdate, "version": 0})


@pytest.mark.parametrize(
    "payload",
    [
        {"name": "a" * 101, "version": 0},
        {"surname": "a" * 101, "version": 0},
        {"about": "a" * 1001, "version": 0},
    ],
)
def test_profile_update_rejects_too_long_text_fields(payload: dict[str, object]) -> None:
    with pytest.raises(ValidationError):
        ProfileUpdateRequest.model_validate(payload)


def test_profile_update_accepts_valid_gender() -> None:
    request = ProfileUpdateRequest.model_validate(
        {
...
def test_profile_update_requires_version() -> None:
    with pytest.raises(ValidationError):
        ProfileUpdateRequest.model_validate({"name": "Andrew"})
...
def test_profile_write_requests_reject_negative_version(
    request_cls: type[ProfileUpdateRequest | EmailChangeRequest | PasswordChangeRequest],
