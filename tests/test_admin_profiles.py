...
from tests.helpers import activate_profile, make_admin, register_and_login
...
async def test_admin_can_search_profiles(client: AsyncClient) -> None:
    ...
    await activate_profile(2, name="User", surname="One", gender="FEMALE")
...
async def test_admin_search_profiles_filters_by_status(client: AsyncClient) -> None:
    ...
    await make_admin(profile_id=1)
    await activate_profile(1, name="Admin", surname="User", gender="MALE")
...
async def test_admin_change_missing_profile_returns_404(
    client: AsyncClient,
) -> None:
    admin_token = await register_and_login(client, email="admin@mail.com")
