...
- `postgres` — PostgreSQL 17 с volume `charm_postgres_data` во внутренней сети;
...
3. Создаёт external-сеть `web` и каталог `/opt/apps/charm`.
4. Рендерит production `.env` и `docker-compose.prod.yml`.
5. Загружает images, запускает PostgreSQL/Redis, Alembic-миграции и seed admin/demo-профилей.
